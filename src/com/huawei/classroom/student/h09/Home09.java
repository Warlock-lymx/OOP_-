...
public class Home09 {
    ...
    public char getFrequentChar(String s) {
        ...
        char[] tmp = s.toCharArray();
        for (char c : tmp) {
            hashMap.put(c, (int) hashMap.getOrDefault(c, 0) + 1);
        }
        int max = 0;
        char res = ' ';
        for (char c : tmp) {
            if ((int) hashMap.get(c) > max) {
                max = (int) hashMap.get(c);
                res = c;
            }
        }
        return res;
    ...
    public String getFrequentWord(String content) {
        ...
        for (int i = 0; i < content.length() - 1; i++) {
            ...
            map.put(str, (int) map.getOrDefault(str, 0) + 1);
        }
        int max = 0;
        String res = "";
        for (Object obj : map.keySet()) {
            if ((int) map.get(obj) > max) {
                max = (int) map.get(obj);
                res = (String) obj;
            }
        }
